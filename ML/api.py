...
def get_dropdown_options():
    """Return metadata/dropdown options for the medicine recommendation form"""
    # Define all categorical features and their options
    metadata = {
        'Target': {
            'type': 'categorical',
            'options': [
                'Type 2 Diabetes', 'Type 1 Diabetes', 'Prediabetic',
                'Gestational Diabetes', 'LADA', 'MODY',
                'Steroid-Induced Diabetes', 'Neonatal Diabetes Mellitus (NDM)',
                'Wolfram Syndrome', 'Wolcott-Rallison Syndrome',
                'Secondary Diabetes', 'Type 3c Diabetes (Pancreatogenic Diabetes)',
                'Cystic Fibrosis-Related Diabetes (CFRD)'
            ]
        },
        'Genetic Markers': {'type': 'categorical', 'options': ['Positive', 'Negative']},
        'Autoantibodies': {'type': 'categorical', 'options': ['Positive', 'Negative']},
        'Family History': {'type': 'categorical', 'options': ['Yes', 'No']},
        'Environmental Factors': {'type': 'categorical', 'options': ['Present', 'Absent']},
        'Physical Activity': {'type': 'categorical', 'options': ['High', 'Moderate', 'Low']},
        'Dietary Habits': {'type': 'categorical', 'options': ['Healthy', 'Unhealthy']},
        'Ethnicity': {'type': 'categorical', 'options': ['Low Risk', 'High Risk']},
        'Socioeconomic Factors': {'type': 'categorical', 'options': ['High', 'Medium', 'Low']},
        'Smoking Status': {'type': 'categorical', 'options': ['Smoker', 'Non-Smoker']},
        'Alcohol Consumption': {'type': 'categorical', 'options': ['High', 'Moderate', 'Low']},
        'Glucose Tolerance Test': {'type': 'categorical', 'options': ['Normal', 'Abnormal']},
        'History of PCOS': {'type': 'categorical', 'options': ['Yes', 'No']},
        'Previous Gestational Diabetes': {'type': 'categorical', 'options': ['Yes', 'No']},
        'Pregnancy History': {'type': 'categorical', 'options': ['Normal', 'Complications']},
        'Cystic Fibrosis Diagnosis': {'type': 'categorical', 'options': ['Yes', 'No']},
        'Steroid Use History': {'type': 'categorical', 'options': ['Yes', 'No']},
        'Genetic Testing': {'type': 'categorical', 'options': ['Positive', 'Negative']},
        'Liver Function Tests': {'type': 'categorical', 'options': ['Normal', 'Abnormal']},
        'Urine Test': {'type': 'categorical', 'options': ['Normal', 'Ketones Present', 'Glucose Present', 'Protein Present']},
        'Early Onset Symptoms': {'type': 'categorical', 'options': ['Yes', 'No']},
        'diabetes': {'type': 'categorical', 'options': ['0', '1']},
        # Numeric features
        'Insulin Levels': {'type': 'numeric'},
        'Age': {'type': 'numeric'},
        'BMI': {'type': 'numeric'},
        'Blood Pressure': {'type': 'numeric'},
        'Cholesterol Levels': {'type': 'numeric'},
        'Waist Circumference': {'type': 'numeric'},
        'Blood Glucose Levels': {'type': 'numeric'},
        'Weight Gain During Pregnancy': {'type': 'numeric'},
        'Pancreatic Health': {'type': 'numeric'},
        'Pulmonary Function': {'type': 'numeric'},
        'Neurological Assessments': {'type': 'numeric'},
        'Digestive Enzyme Levels': {'type': 'numeric'},
        'Birth Weight': {'type': 'numeric'},
        'HbA1c_level': {'type': 'numeric'},
        'dosage_mg': {'type': 'numeric'}
    }
...
def predict_medicine(request: MedicinePredictionRequest):
    """
    Predict recommended medicine based on patient features.
    NOTE: This is a placeholder implementation until a trained medicine model is available.
    """
    features = request.features
    
    # Extract key features for basic rule-based recommendation
    target = features.get('Target', 'Type 2 Diabetes')
    hba1c = float(features.get('HbA1c_level', 0))
    glucose = float(features.get('Blood Glucose Levels', 0))
    bmi = float(features.get('BMI', 0))
    
    # Simple rule-based medicine recommendation (placeholder logic)
    if target == 'Type 1 Diabetes':
        medicine = 'Insulin (Long-acting)'
        dosage = 10
        confidence = 0.85
    elif target == 'Type 2 Diabetes':
        if hba1c > 9:
            ...
            dosage = 0
            confidence = 0.70
    elif target == 'Gestational Diabetes':
        medicine = 'Insulin (Short-acting)'
        dosage = 5
        confidence = 0.75
    else:
        medicine = 'Metformin'
    ...
    return {
        ...
        "dosage_mg": dosage,
        "medicine_confidence": confidence,
        "dosage_confidence": confidence * 0.9,
        "all_medicine_probabilities": all_probabilities,
        "note": "This is a rule-based placeholder. Train and deploy a medicine recommendation model for accurate predictions."
    }

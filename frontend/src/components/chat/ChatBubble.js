...
export default function ChatBubble({
  ...
  const actualIsUser =
    typeof isUser !== "undefined" ? isUser : message?.role === "user";
  ...
  return (
    <div
      className={`chat-bubble-group ${actualIsUser ? "user" : actualIsError ? "error" : "assistant"}`}
    >
      <div
        className={`bubble ${actualIsUser ? "user" : actualIsError ? "error" : "assistant"}`}
      >
        {actualIsError && (
        ...
        <span className="message-time">
          {actualTimestamp.toLocaleTimeString("en-US", {
            hour: "2-digit",
            minute: "2-digit",
          })}
        </span>

...
export default function HealthJourneyAI({ patient, visits }) {
  ...
  const getStatusColor = (status) => {
    ...
    if (s.includes("warning") || s.includes("fair") || s.includes("moderate"))
      ...
      <div className="health-journey-card">
        <div className="health-journey-content">
          ...
          <p className="mb-0 opacity-75">
            Add some visits with predictions to get personalized AI advice on
        ...
        <div className="d-flex justify-content-between align-items-start mb-3">
          <div>
            ...
            <small className="opacity-75">
              Analyzing {visits.length} visit{visits.length !== 1 ? "s" : ""} to
                ...
                {aiContent.split("\n").map((line, i) => {
                  ...
                  if (line.trim().match(/^\d+\./)) {
                    ...
                    return (
                      <div
                        key={i}
                        className="d-flex align-items-start mb-2 small"
                      >
                  ...
                  <div className="row g-3 mb-4">
                    {aiContent.key_takeaways.map((item, idx) => (
                      <div key={idx} className="col-md-6 col-lg-4">
                        <div
                          className={`p-3 rounded-3 h-100 ${getStatusColor(item.status)} border border-0`}
                        >
                          <div className="d-flex align-items-center mb-2">
                            <i
                              className={`${getStatusIcon(item.status)} me-2`}
                            ></i>
                  ...
                  <div className="col-md-7">
                    ...
                    <div className="d-flex flex-column gap-3">
                      {aiContent.personalized_advice?.map((advice, idx) => (
                        <div
                          key={idx}
                          className="d-flex align-items-start bg-black bg-opacity-25 p-3 rounded-2"
                        >
                          <div className="me-3 mt-1">
                            <i className="fas fa-lightbulb text-warning"></i>
                          </div>
                          <div>
                            <div className="fw-bold small text-uppercase opacity-75 mb-1">
                              {advice.category}
                            </div>
                            <p className="mb-1 fw-semibold">{advice.action}</p>
                            <small className="opacity-75">
                              <i className="fas fa-arrow-right me-1 small"></i>
                              {advice.impact}
                            </small>
                          </div>
                        </div>
                  ...
                  <div className="col-md-5">
                    ...
                    <ul className="list-group list-group-flush bg-transparent">
                      {aiContent.next_steps?.map((step, idx) => (
                        <li
                          key={idx}
                          className="list-group-item bg-transparent text-white px-0 d-flex"
                        >
